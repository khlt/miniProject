				...
				<!-- Two -->
					<section id="two" class="wrapper style3 fade-up">
						<div class="inner">
							...
							<div class="features">
								...
								<div class="panel-body">
									<table width="100%" class="table table-striped table-bordered table-hover" >
										...
										<tr>
										<td><c:out value="${Reservation.reserveno}"/></td>
										<c:if test="${ (login.userName == Reservation.writer) || (login.userId == 'admin')}">
										<td><a href='/reservation/get?reserveno=<c:out value="${Reservation.reserveno }#two"/>'>                       
										<c:out value="${Reservation.classroom}"/></a></td>
										</c:if>
										
										<c:if test="${ (login.userName != Reservation.writer) && login.userId != 'admin'}">
										<td><c:out value="${Reservation.classroom}"/></td>
										</c:if>
										<td><c:out value="${Reservation.reservedate }"></c:out></td>
										
										<td><c:out value="${Reservation.reservetime }"></c:out></td>
										
										<td><c:out value="${Reservation.writer}"/></td>
										<!-- 위치 수정 -->
							...
							<button id='regBtn' type="button" class="btn btn-xs pull-right">예약신청</button>
...
<!-- Bootstrap Core JavaScript -->
    ...
    $(document).ready(function(){
       ...
       $("#regBtn").on("click", function(){
          self.location = "/reservation/register2#two";
       });
       console.log('result',result);
    });
    </script>
